    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm transition-opacity duration-200">
      <div className="w-full max-w-4xl glass-modal rounded-sm p-6 sm:p-10 lg:p-16 relative overflow-y-auto max-h-[90vh] shadow-2xl min-h-[400px] sm:min-h-[500px] animate-in fade-in zoom-in-95 duration-200 ease-out">
        ...
        <div className={`absolute inset-0 flex flex-col items-center justify-center p-6 sm:p-16 transition-opacity duration-300 z-10 ${syncing ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-10 w-full opacity-5">
            <div className="h-32 sm:h-40 w-full skeleton-shimmer"></div>
            <div className="h-32 sm:h-40 w-full skeleton-shimmer hidden sm:block"></div>
          </div>
        </div>
        ...
        <div className={`transition-all duration-300 ${syncing ? 'opacity-0 scale-98' : 'opacity-100 scale-100'}`}>
          <div className="mb-8 sm:mb-14 text-center">
            <span className="mono text-[8px] sm:text-[9px] font-black tracking-[0.4em] sm:tracking-[0.6em] text-[#7B2CFF] uppercase block mb-3 sm:mb-4 opacity-60">PHILOSOPHY</span>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-black dark:text-white tracking-tighter">Intelligence, Architected.</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-12">
            <div className="space-y-6 sm:space-y-8">
              <p className="text-base sm:text-lg lg:text-xl text-black dark:text-white font-light leading-relaxed tracking-tight border-l-2 border-[#18E6FF] pl-4 sm:pl-6 lg:pl-8">
                The user remains the central architect, with AI acting as the weaver of intent.
              </p>
              <p className="text-xs sm:text-sm text-black dark:text-white/60 font-medium italic">
                Architecting intelligent operating systems that synthesize data streams into human intent.
              </p>
            </div>

            <div className="space-y-4 sm:space-y-6">
              <div className="p-4 sm:p-6 bg-black/10 dark:bg-white/5 rounded-sm border border-black/5 dark:border-white/5">
                <h3 className="mono text-[8px] sm:text-[9px] font-black tracking-widest text-[#7B2CFF] uppercase mb-2">Agency</h3>
                <p className="text-black/70 dark:text-white/50 text-[11px] sm:text-xs leading-relaxed">
                  Systems empower, not replace. Every workflow is transparent and modular.
                </p>
              </div>
              <div className="p-4 sm:p-6 bg-black/10 dark:bg-white/5 rounded-sm border border-black/5 dark:border-white/5">
                <h3 className="mono text-[8px] sm:text-[9px] font-black tracking-widest text-[#18E6FF] uppercase mb-2">Autonomy</h3>
                <p className="text-black/70 dark:text-white/50 text-[11px] sm:text-xs leading-relaxed">
